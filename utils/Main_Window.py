...
class DetectThread(QtCore.QThread):
    ...
    def __init__(self, model: detect.YOLOv11 = None, dataset: detect.DataLoader = None):
...
class MainWindow(QtWidgets.QMainWindow, Yolo2onnx_detect_Demo_UI.Ui_MainWindow):
    ...
    def UI(self):  # 槽函数
        ...
        self.toolButton_4.clicked.connect(lambda: self.changeClassFile(None))  # 选择类别
    ...
    def loadConfig(self):  # 加载配置
        ...
        self.lineEdit_3.setText(cfg.search('root', 'model_path', os.path.join('need', 'models', 'yolov11.onnx')))
        self.class_file = cfg.search('root', 'class_path', os.path.join('need', 'yolov11.txt'))
